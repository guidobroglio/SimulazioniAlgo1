...
static int test_int_bst(int *keys, int *values, size_t n, int *key, int nth);
static int test_str_bst(char **keys, int *values, size_t n, char **key, int nth);
static int test_bst(void *keys, size_t key_sz, void *values, size_t value_sz, size_t n, void *key, int nth, upo_bst_comparator_t key_cmp, char* (*key_to_string)(const void*), char* (*value_to_string)(const void*));
extern void* upo_bst_xyz(upo_bst_t, void*, int);
...
int test_bst1(void)
{
    ...
    return test_int_bst(keys, values, n, &key, nth);
}
...
int test_bst12(void)
{
    ...
    return test_str_bst(keys, values, n, &key, nth);
}
...
int test_int_bst(int *keys, int *values, size_t n, int *key, int nth)
{
    return test_bst(keys, sizeof keys[0], values, sizeof values[0], n, key, nth, upo_test_int_cmp, upo_test_int_to_string, upo_test_int_to_string);
}

int test_str_bst(char **keys, int *values, size_t n, char **key, int nth)
{
    return test_bst(keys, sizeof keys[0], values, sizeof values[0], n, key, nth, upo_test_str_cmp, upo_test_str_to_string, upo_test_int_to_string);
}

int test_bst(void *keys, size_t key_sz, void *values, size_t value_sz, size_t n, void *key, int nth, upo_bst_comparator_t key_cmp, char* (*key_to_string)(const void*), char* (*value_to_string)(const void*))
{
    upo_bst_t bst = NULL;
    ...
    if (n > 0)
    {
        ...
        /* Find n-th minimum key */
        void *k = NULL;
        void *check_k = NULL; 

        k = upo_bst_nmin(bst, key, nth);
        check_k = upo_bst_xyz(bst, key, nth);
        ...
        {   
            if (key_cmp(k,check_k)!=0){
                ...
                fprintf(stderr, "[file: %s, line: %d] ERROR: %d-th minimum key of tree with root '%s' -> Expected '%s', got '%s'.\n", __FILE__, __LINE__, nth,key_str,check_str,k_str);
                ...
                fprintf(stderr, "[file: %s, line: %d] ERROR: %d-th minimum key of tree with root '%s' -> Expected '%s', got none.\n", __FILE__, __LINE__, nth,key_str, check_str);
                fprintf(stderr, "[file: %s, line: %d]: <BST>\n", __FILE__, __LINE__);
                upo_bst_dump(bst, stderr, key_to_string, 1, value_to_string, 1);
                fprintf(stderr, "[file: %s, line: %d]: </BST>\n", __FILE__, __LINE__);
                fflush(stderr);
                ...
                char *key_str = key_to_string(key);
                char *k_str = key_to_string(k);
                fprintf(stderr, "[file: %s, line: %d] ERROR: %d-th minimum key of tree with root '%s' -> Expected NULL, got '%s'.\n", __FILE__, __LINE__, nth,key_str, k_str);
                fprintf(stderr, "[file: %s, line: %d]: <BST>\n", __FILE__, __LINE__);
                upo_bst_dump(bst, stderr, key_to_string, 1, value_to_string, 1);
                fprintf(stderr, "[file: %s, line: %d]: </BST>\n", __FILE__, __LINE__);
                fflush(stderr);
    ...
    else
    {
        ...
        void *k = upo_bst_nmin(bst, key, nth);
