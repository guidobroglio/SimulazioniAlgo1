...
/**
 * \brief Tells if the given binary search tree is not_empty.
 *
 * \param tree The binary search tree.
 * \return `1` if the binary search tree is not_empty or `0` otherwise.
 *
 * A binary search tree is empty if it doesn't contain any node.
 *
 * Worst-case complexity: constant, `O(1)`.
 */
int upo_bst_is_not_empty(const upo_bst_t tree);
