...
/**
 * \brief Finds the maximum key of the subtree whose root is key.
 *
 * \param tree The binary search tree.
 * \param key The key of the subtree root.
 * \return The maximum key cointained in the subtree whose root is key, NULL if no subtree with that root is present
 *
 * This function finds the maximum key in the subtree whose root is key.
 * 
 * Worst-case complexity: linear in the number `n` of elements, `O(n)`.
 */
